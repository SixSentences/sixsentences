...
estimate of works not captured under the model assumptions, with a 95%
confidence interval. Completeness is then observed / estimated.

This is a statement about the SEARCH STRATEGY over the pinned corpus, not about
all of science. With fewer than two occasions, fewer than two singletons, or no
externally established basis for treating occasions as independent, the
estimate is undetermined rather than spuriously exact.
...
class CoverageReport(BaseModel):
    """Capture-recapture estimate with explicit uncertainty and inputs."""

    method: str  # "chao2" | "undetermined"
    observed: int
    estimated_total: float | None
    completeness: float | None  # observed / estimated_total, in (0, 1]
    ci_low: float | None  # completeness CI (from the estimated-total CI)
    ci_high: float | None
    occasions: int
...
def estimate_completeness(
    ...
    occasions: int,
    *,
    occasion_independence_verified: bool = False,
) -> CoverageReport:
    ...
    `occasions` is the number of queries executed. The independence flag is a
    caller assertion; this function cannot infer independence from overlap.
    """
    if isinstance(occasions, bool) or not isinstance(occasions, int) or occasions < 0:
        raise ValueError("occasions must be a non-negative integer")
    if not isinstance(occasion_independence_verified, bool):
        raise ValueError("occasion_independence_verified must be a boolean")
    for work_id, count in capture_counts.items():
        if not isinstance(work_id, str) or not work_id.strip():
            raise ValueError("capture-count work ids must be non-empty strings")
        if isinstance(count, bool) or not isinstance(count, int):
            raise ValueError(f"capture count for {work_id!r} must be an integer")
        if count < 1 or count > occasions:
            raise ValueError(
                f"capture count for {work_id!r} must be between 1 and occasions ({occasions})"
            )
    ...
    if occasions < 2 or observed == 0:
        reason = "need >=2 searches with observed captures to estimate completeness"
    elif not occasion_independence_verified:
        reason = "occasion independence has not been externally verified"
    elif f1 < 2:
        reason = "fewer than two singleton captures cannot support an unseen-work estimate"
    ...
    return CoverageReport(
        ...
        doubletons=f2,
        note=(
            f"estimated {int(round(f0))} works were not captured under the stated "
