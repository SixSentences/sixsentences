...
Two roles:
1. Cursor-paged ingestion for a caller-owned corpus.
2. Public metadata search with a compiled boolean string.

Etiquette/auth: sends `mailto` and `api_key` when configured. OpenAlex permits
a few anonymous demo calls, while sustained and production use requires or
strongly benefits from a free key. Cursor paging follows provider docs.
"""
...
_ARXIV_RE = re.compile(
    r"arxiv\.org/(?:abs|pdf)/(\d{4}\.\d{4,5}|[a-z\-]+(?:\.[A-Z]{2})?/\d{7})", re.IGNORECASE
)


class OpenAlexError(RuntimeError):
    """Raised when OpenAlex rejects or repeatedly fails a request."""
...
def _validate_year_range(year_from: int | None, year_to: int | None) -> None:
    for name, year in (("year_from", year_from), ("year_to", year_to)):
        if year is not None and (
            isinstance(year, bool) or not isinstance(year, int) or not 0 <= year <= 9999
        ):
            raise ValueError(f"{name} must be an integer between 0 and 9999")
...
def _parse_oa(data: dict[str, Any]) -> dict[str, Any]:
    ...
    OpenAlex ingests Unpaywall, so ``best_oa_location`` carries its preferred
    open-access link without requiring a separate Unpaywall call.
    """
...
class OpenAlexClient:
    """Bounded synchronous client for public OpenAlex work metadata."""
    ...
    def _get_works(self, params: dict[str, str | int]) -> dict:  # type: ignore[type-arg]
        ...
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                if response.status_code in (401, 403):
                    raise OpenAlexError(
                        "OpenAlex rejected the request. Set or check OPENALEX_API_KEY; "
                        "anonymous access is limited to a few demo calls."
                    )
        ...
        raise OpenAlexError(f"OpenAlex request failed after {_MAX_RETRIES} attempts") from None
    ...
    def iter_works(
        ...
        *,
        ...
        if not oa_filter.strip():
            raise ValueError("oa_filter must not be empty")
