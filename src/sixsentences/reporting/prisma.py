"""PRISMA 2020 flow rendering from caller-supplied counters.

This module formats validated explicit counts; it does not infer them from a
workflow or claim that the surrounding review is compliant. PRISMA-S-style
search documentation is emitted separately from ``SearchExecution`` records.
"""
...
def render_flow_text(counts: PrismaCounts) -> str:
    """Render conserved, caller-supplied flow counters as plain text."""

    lines = [
        "PRISMA 2020 flow",
        "  Identification",
        f"    records identified:        {counts.records_identified}",
        f"    via other methods:         {counts.other_identified}",
        f"    via citation search:       {counts.citation_identified}",
        f"    duplicates removed:        {counts.duplicates_removed}",
        f"    companion reports linked:  {counts.companion_reports_linked}",
        "  Screening",
    ...
    if counts.reports_assessed_for_eligibility:
        lines += [
            ...
            f"    reports included:          {counts.reports_included}",
...
def render_flow_svg(counts: PrismaCounts) -> str:
    """Render caller-supplied PRISMA 2020 flow counters as an SVG.

    Stages that did not run are absent rather than zero-padded. The renderer
    formats conserved counters but does not establish PRISMA compliance.
    """
    ...
    if counts.other_identified:
        identification_parts.append(f"{counts.other_identified:,} via other methods")
    if counts.citation_identified:
        identification_parts.append(f"{counts.citation_identified:,} via citation search")
    if counts.companion_reports_linked:
        identification_parts.append(f"{counts.companion_reports_linked:,} companion report links")
    if identification_parts:
        identified_label += " (" + ", ".join(identification_parts) + ")"
    ...
    if has_retrieval:
        rows.append(
            (
                "Retrieval",
                (counts.reports_sought_for_retrieval, "reports sought", "main"),
                (counts.reports_not_retrieved, "reports not retrieved"),
            )
        )
    if has_eligibility:
        rows.append(
        ...
        rows.append(
            ("Reports", (counts.reports_included, "reports of included studies", "final"), None)
        )
    ...
    parts = [
        ...
        f'fill="{_PINE}" font-family="Courier New, monospace">'
        "PRISMA 2020 FLOW — CALLER-SUPPLIED</text>",
