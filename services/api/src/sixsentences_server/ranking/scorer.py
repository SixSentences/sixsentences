...
so a user (and the audit log) can see WHY a work ranks where it does — ranking
is never an opaque number (VISION.md #1/#6, CONCEPT.md stage 6).
...
- **relevance** — lexical coverage of the protocol vocabulary (question +
  ...
  weighted higher. This is an honest offline proxy; H1 swaps in embedding
  similarity behind the same interface without touching callers.
...
_FIELD_PREFIX = re.compile(r"\b\w+:")
...
def _query_vocabulary(protocol: ReviewProtocol) -> set[str]:
    ...
    Boolean operators, field prefixes, wildcards and quoting are stripped so
    the vocabulary is real domain words, not query syntax.
    """
    raw = " ".join([protocol.question, *protocol.inclusion_criteria, protocol.query_string])
    raw = _BOOL.sub(" ", raw)
    raw = _FIELD_PREFIX.sub(" ", raw)
    for ch in '*"()':
        raw = raw.replace(ch, " ")
    return _terms(raw)
